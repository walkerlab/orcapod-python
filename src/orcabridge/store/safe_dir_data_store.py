    ...
    try:
        while True:
            try:
                        ...
                        try:
                            with open(lock_file, 'r') as f:
                                ...
                                if ',' in content:
                                    ...
                                    if not process_exists or time.time() - lock_time > stale_threshold:
                ...
                logger.debug(f"Error while trying to acquire lock {lock_file}: {str(e)}")
        ...
        logger.debug(f"Acquired {'shared' if shared else 'exclusive'} lock on {lock_file}")
        ...
        if fd is not None:
            ...
            logger.debug(f"Released {'shared' if shared else 'exclusive'} lock on {lock_file}")
...
class SafeDirDataStore:
    ...
    def memoize(
        ...
        with file_lock(lock_path, shared=False, timeout=self.lock_timeout, 
                       stale_threshold=self.lock_stale_threshold):
            # Double-check if the entry already exists (another process might have created it)
            if completion_marker.exists() and not self.overwrite:
                logger.info(f"Entry already exists for packet {packet} (verified with exclusive lock)")
                return self.retrieve_memoized(store_name, content_hash, packet)
            ...
            if partial_marker.exists():
                ...
                if time.time() - partial_time > self.lock_stale_threshold:
                    logger.warning(f"Found stale partial results in {output_dir}, cleaning up")
            ...
            try:
                ...
                return self._retrieve_without_lock(store_name, content_hash, packet, output_dir)
    ...
    def _retrieve_without_lock(
        ...
        if not info_path.exists():
            logger.warning(f"Completion marker exists but info file missing for {packet}")
            return None
