...
class DeltaTableArrowDataStore:
    ...
    def __init__(
        ...
        if create_base_path:
            ...
            raise ValueError(f"Base path {self.base_path} does not exist and create_base_path=False")
    ...
    def _validate_source_path(self, source_path: tuple[str, ...]) -> None:
        ...
        if len(source_path) > self.max_hierarchy_depth:
            raise ValueError(f"Source path depth {len(source_path)} exceeds maximum {self.max_hierarchy_depth}")
        
        # Validate path components
        for i, component in enumerate(source_path):
            if not component or not isinstance(component, str):
                raise ValueError(f"Source path component {i} is invalid: {repr(component)}")
            
            # Check for filesystem-unsafe characters
            unsafe_chars = ['/', '\\', ':', '*', '?', '"', '<', '>', '|', '\0']
            if any(char in component for char in unsafe_chars):
                raise ValueError(f"Source path component contains invalid characters: {repr(component)}")
        ...
        try:
            ...
            if self.duplicate_entry_behavior == "overwrite":
                ...
                try:
                    ...
                    logger.debug(f"Deleted existing record {entry_id} from {source_key}")
    ...
    def get_record(
        ...
        try:
            ...
            logger.error(f"Error getting record {entry_id} from {'/'.join(source_path)}: {e}")
            return None
    ...
    def get_records_by_ids(
        ...
        try:
            ...
            logger.error(f"Error getting records by IDs from {'/'.join(source_path)}: {e}")
            return None
    ...
    def delete_record(self, source_path: tuple[str, ...], entry_id: str) -> bool:
        ...
        try:
            ...
            existing = delta_table.to_pyarrow_table(filter=f"__entry_id = '{escaped_entry_id}'")
            if len(existing) == 0:
                return False
            ...
            logger.error(f"Error deleting record {entry_id} from {'/'.join(source_path)}: {e}")
            return False
