...
class ArrowBatchedPolarsDataStore:
    ...
    def _consolidate_arrow_batch(self, source_key: str) -> None:
        ...
        logger.debug(f"Consolidating {len(self._arrow_batches[source_key])} Arrow tables for {source_key}")
        ...
        logger.debug(f"Consolidated to Polars DataFrame with {len(self._polars_store[source_key])} total rows")
        ...
        if entry_id in self._entry_index[source_key]:
            if self.duplicate_entry_behavior == "error":
                ...
                self._arrow_batches[source_key] = [
                    (eid, table) for eid, table in self._arrow_batches[source_key]
                    if eid != entry_id
                ]
                
                # Remove from Polars store if it exists
                if source_key in self._polars_store:
                    self._polars_store[source_key] = self._polars_store[source_key].filter(
                        pl.col("__entry_id") != entry_id
                    )
    ...
    def get_records_by_ids(
        ...
        existing_entries = [
            entry_id for entry_id in entry_ids_list 
            if entry_id in self._entry_index[source_key]
        ]
        ...
        if add_entry_id_column is False:
            ...
            if "__entry_id" in column_names:
                indices = [i for i, name in enumerate(column_names) if name != "__entry_id"]
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        total_consolidated = sum(
            len(df) for df in self._polars_store.values()
        ) if self._polars_store else 0
